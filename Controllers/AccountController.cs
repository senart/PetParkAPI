...
using PetPark.Providers;
using PetPark.Results;
...
namespace PetPark.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        // POST api/Account/ProfilePicture
        ...
        public async Task<IHttpActionResult> ProfilePicture()
        {
            ...
            var uploadedFile = await _fileService.UploadSingleFile(Request.Content);
            if (uploadedFile != null)
            {
                var currentUser = await UserManager.FindByIdAsync(User.Identity.GetUserId());
                if (currentUser.FileModelURL != null || currentUser.FileModelURL != "")
                {
                    _fileService.DeleteFile(currentUser.FileModelURL);
                }
                currentUser.FileModelURL = uploadedFile.FileUrl;

                var result = await UserManager.UpdateAsync(currentUser);

                if (!result.Succeeded) 
                {
                    return InternalServerError(new Exception("User update not succesfull"));
                }
        ...
        {
            return _petService.GetPetsForUser(User.Identity.GetUserId());
        }
        ...
        public async Task<IHttpActionResult> GetExternalLogin(string provider, string error = null)
        {
            ...
            if (hasRegistered)
            {
                ...
                AuthenticationProperties properties = ApplicationOAuthProvider.CreateProperties(user.UserName, user.FileModelURL ?? "");
